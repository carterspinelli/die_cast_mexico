                  ...
                  <StyledTable>
                    ...
                    <tbody>
                      {tab.items.map((item, index) => {
                        ...
                        // Set Spanish translations based on the current language
                        // Using language from the component's useLanguage() call at the top level
                        const isSpanish = language === "es";
                        if (isSpanish) {
                          if (tab.id === "overview") {
                            // Overview tab translations
                            if (item.attribute === (translations.workforce || "Workforce")) {
                              displayValue = "125+";
                            } else if (item.attribute === (translations.materialCapacity || "Material Capacity")) {
                              displayValue = "Hasta 250 toneladas/mes de lingotes de aluminio";
                            } else if (item.attribute === (translations.aluminumAlloys || "Aluminum Alloys")) {
                              displayValue = "AlSi12(Fe), A380, A360, A413, ADC12";
                            }
                          } else if (tab.id === "manufacturing") {
                            // Manufacturing tab translations
                            if (item.attribute === (translations.dieCasting || "Die Casting")) {
                              displayValue = "8 celdas de fabricación automáticas";
                            } else if (item.attribute === (translations.cncMachining || "CNC Machining")) {
                              displayValue = "19 máquinas horizontales de 4 ejes";
                            } else if (item.attribute === (translations.mechanicalAssembly || "Mechanical Assembly")) {
                              displayValue = "Pasadores, etiquetas, helicoides, etc.";
                            }
                          } else if (tab.id === "finishing") {
                            // Surface Finishing tab translations
                            if (item.attribute === (translations.fipGasket || "FIP Gasket")) {
                              displayValue = "Aplicación de juntas de precisión";
                            } else if (item.attribute === (translations.powderPainting || "Powder Painting")) {
                              displayValue = "Akzo Nobel, Cardinal, Sherwin Williams, etc.";
                            } else if (item.attribute === (translations.liquidPainting || "Liquid Painting")) {
                              displayValue = "Soluciones de color personalizadas"; 
                            } else if (item.attribute === (translations.nickelPlating || "Nickel Plating")) {
                              displayValue = "Acabado superficial de alta calidad";
                            } else if (item.attribute === (translations.surfaceTreatment || "Surface Treatment")) {
                              displayValue = "Pasivación de tri-cromo en Aluminio: Surtec 650®";
                            }
                          }
                        }
                          ...
                          >
                            ...
                            <td style={{ padding: '1rem' }}>
                              <div style={{ 
                                ...
                                <SecondaryCode>{displayValue}</SecondaryCode>
                              </div>
                            </td>
                          </tr>
                        );
